...
|  | 命令词识别进阶要求（GRU, Test Acc 85.2% > 80%） |
...
仅将 `nn.RNN` 替换为 `nn.GRU`，测试准确率从 **25.2% 跃升至 79.2%**。

1. **快速收敛**：第 1 轮验证准确率即达 31.0%（RNN 仅 11.0%），第 3 轮已超过 60%。
...
仅将 `num_epochs` 从 10 增至 20，测试准确率飙升至 **85.2%**，大幅超过 80% 目标。

1. **持续优化**：Loss 从 0.45（10 轮）降至 0.178（20 轮），模型继续收敛。
2. **验证集稳定超 80%**：第 12 轮起验证准确率多次达到 79.8%~80.8%，表明模型泛化能力良好。

### 4.4 对比总结

| 对比项 | RNN（基础） | GRU（进阶） | GRU+20轮（最佳） |
|--------|------------|------------|-----------------|
| **测试准确率** | 25.2% | 79.2% | **85.2%** |
| 首轮验证准确率 | 11.0% | 31.0% | 31.0% |
| 最终 Loss | 2.77 | 0.45 | **0.178** |
| 训练轮数 | 10 | 10 | 20 |
| 每轮耗时 | ~7.2s | ~6.9s | ~7.4s |

![准确率对比](https://files.seeusercontent.com/2026/05/12/Q0to/acc_comparison.png)

### 4.5 模型选择与参数说明

| 参数 | 选择理由 |
|------|----------|
| **GRU 替代 RNN** | 门控机制有效缓解梯度消失，本实验准确率从 25.2% 提升至 85.2% |
| `hidden_size=256` | 平衡模型容量和计算开销，SpeechCommands 数据量不大，256 已足够 |
| `num_layers=4` | 多层 GRU 能提取更高层次的时序特征，4 层在验证集上表现最优 |
| `dropout=0.2` | 防止过拟合，训练集约 10000 条，适中 Dropout 保持泛化 |
| `batch_size=256` | RTX 4070 显存充裕（12GB），大 batch 加速训练 |
| `lr=0.001` | Adam 优化器默认学习率，固定学习率比余弦退火效果更好 |
| `num_epochs=20` | 10 轮验证集仍在上升（77%），增加至 20 轮后测试集达 85.2% |
...
**验证集最低 CER：10.94%**（第 9 轮），满足 < 40% 要求。
...
| RNN 训练准确率仅 25.2% | 基础 RNN 梯度消失 | 替换为 GRU，准确率跃升至 85.2% |
| 测试集 CER 高达 101% | 预训练模型过拟合，训练集和测试集分布不一致 | 使用预训练-微调 + 更多轮数微调 |
| CosineAnnealingLR 不如固定学习率 | 后期学习率过低，模型难以跳出局部最优 | 改用固定学习率 0.001 训练 20 轮 |

### 8.2 主要收获

1. **深入理解 RNN 与 GRU 的区别**：通过实际代码验证了 GRU 门控机制对梯度消失的缓解作用，从 25.2% 到 85.2% 的跃升是理论与实践结合的生动案例。
